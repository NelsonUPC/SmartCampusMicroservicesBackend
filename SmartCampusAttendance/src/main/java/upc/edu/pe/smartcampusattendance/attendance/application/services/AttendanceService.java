...
public class AttendanceService {
    ...
    public AttendanceSession updateFullSession(Long sessionId, AttendanceSessionRequest request) {
        AttendanceSession session = sessionRepo.findById(sessionId)
                .orElseThrow(() -> new RuntimeException("Session not found"));

        // Actualizar los datos básicos de la sesión si es necesario
        session.setCourseId(request.getCourseId());
        session.setClassroomId(request.getClassroomId());
        session.setTeacherId(request.getTeacherId());
        session.setDate(request.getDate());

        // Actualizar o agregar asistencias de alumnos
        List<StudentAttendance> currentAttendances = session.getStudentAttendances();
        for (StudentAttendanceDTO dto : request.getAttendances()) {
            ...
            if (existing != null) {
                ...
                StudentAttendance newAttendance = StudentAttendance.builder()
                        ...
                        .session(session)
                        .build();
                studentRepo.save(newAttendance);
                currentAttendances.add(newAttendance);
            }
        }
        session.setStudentAttendances(currentAttendances);
        AttendanceSession updated = sessionRepo.save(session);
        return updated;
    }
